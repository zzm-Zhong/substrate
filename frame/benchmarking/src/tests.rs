...
mod benchmarks {
	...
	use crate::{account, BenchmarkParameter, BenchmarkResult, BenchmarkReturn, BenchmarkingSetup};
	use frame_support::{assert_err, assert_ok, dispatch::Dispatchable, ensure, traits::Get};
	...
	crate::benchmarks! {
		...
		override_benchmark {
			let caller = account::<T::AccountId>("caller", 0, 0);
			let call = crate::tests::Call::TestPallet(
				crate::tests::pallet_test::Call::always_error()
			);
		}: {
			call.dispatch(RawOrigin::Signed(caller).into())
				.unwrap_or_else(|_|
					return BenchmarkReturn::Override(
						BenchmarkResult {
	...
	fn benchmark_override_works() {
		...
		new_test_ext().execute_with(|| {
			let result = closure();
			println!("shawn {:?}", result);
			assert!(matches!(result, Ok(BenchmarkReturn::Override(_))));
		});
	}
